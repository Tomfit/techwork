...
const signup = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(email,
         ...
         `Hello ${name} \n\nThank you for signing Up! Your account has been created successfully\n\n Verify your email with this token ${emailtoken}
         \n\nBest regards\n\n Your service team`,)
...
const verifyEmail = async (req,res) => {
    ...
    if(!token){
        ...
        if (!user){
            return res.status(404).json({messgae: "User with this token doesn't exist"})
        }
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Password reset token generated",otp,});
